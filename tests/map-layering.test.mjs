...
assert.match(html, /name:'Paşabağ[^']*'[\s\S]{0,220}region:'cappadocia'[\s\S]{0,220}time:'9\/28[^']*'/, 'the Cappadocia map should include the September 28 fairy-chimney stop');
assert.match(html, /name:'Devrent[^']*'[\s\S]{0,220}region:'cappadocia'[\s\S]{0,220}time:'9\/28[^']*'/, 'the Cappadocia map should include the September 28 imagination-valley stop');
assert.match(html, /name:'Avanos[^']*(?:陶艺|Pottery)[^']*'[\s\S]{0,220}region:'cappadocia'[\s\S]{0,220}time:'9\/28[^']*'/i, 'the Cappadocia map should include the September 28 pottery stop');
assert.match(html, /(?:北线地貌|Red Tour)\s*＋?\s*Avanos\s*陶艺(?:小团|团)[\s\S]{0,220}9\/28[^<]{0,30}周一[\s\S]{0,260}(?:09:30|9:30)[^<]{0,80}16:00[\s\S]{0,300}18:30[^<]{0,80}(?:ASR|格雷梅)/, 'the map-side route card should show the fixed group-tour window and airport deadline');
